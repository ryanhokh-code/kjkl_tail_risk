"""
Centralized configuration for the kjkl_tail_risk project.

Single source of truth for market universes, ticker lists, file paths,
default parameters, and other constants shared across modules.
"""
...
PARAM_FILE = os.path.join(PROJECT_DIR, 'optimized_params.json')
...
# ---------------------------------------------------------------------------
# Market universes — one dict to rule them all
# ---------------------------------------------------------------------------
MARKET_UNIVERSES = {
    '^GSPC': {
        'name': 'US (S&P 500)',
...
def generate_default_market_tickers(market_ticker):
    """Return the default constituent ticker list for a given market index."""
    if market_ticker in MARKET_UNIVERSES:
        return MARKET_UNIVERSES[market_ticker]['tickers']
    elif market_ticker in ('^NDX', '^IXIC'):
        return [
            'AMZN', 'AAPL', 'MSFT', 'META', 'GOOGL', 'NVDA', 'TSLA',
            'AVGO', 'COST', 'PEP', 'CSCO', 'TMUS', 'NFLX', 'AMD',
            'INTC', 'CMCSA', 'QCOM', 'TXN', 'HON', 'AMGN'
        ]
    else:
